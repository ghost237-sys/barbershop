  ...
  if (!entry) {
    return (
      <div className="w-full rounded-2xl border-2 border-dashed border-zinc-700
        ...
        <p className="text-zinc-400 font-semibold text-lg">No one in the chair</p>
        <p className="text-zinc-600 text-sm mt-1">Kiti kiko wazi</p>
      </div>
    )
  }
    ...
    `}>
      ...
      <div className="flex items-center justify-between mb-4">
        <span className={`text-xs font-bold uppercase tracking-widest
          ${overtime ? 'text-red-400' : 'text-amber-400'}`}>
          {overtime ? '⚠️ Running Long!' : 'Now Serving / Anahudumika'}
        </span>
      ...
      <div className="flex items-center gap-4">
        <div className={`w-16 h-16 rounded-full flex items-center justify-center
                         ...
                         ${overtime ? 'bg-red-400 text-white' : 'bg-amber-400 text-zinc-900'}`}>
      ...
      {overtime && (
        <div className="mt-4 bg-red-400/10 rounded-xl px-4 py-2 text-center">
          <p className="text-red-400 text-sm font-semibold">
            This cut is running over {avgMinutes} minutes
          </p>
